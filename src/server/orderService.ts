...
/**
 * Validates the raw request payload strictly against security constraints.
 * - Enforces fixed price = 88 JOD regardless of any client input.
 * - Total = quantity * 88.
 * - Restricts selectedColor strictly to: "برتقالي" | "سكني" | "أسود".
 * - Ensures required fields are present and valid.
 */
export function validateOrder(payload: OrderPayload): ValidationResult {
  ...
  // Simple phone check: at least 8 digits/chars
  ...
  const governorate =
    typeof payload.governorate === "string" ? payload.governorate.trim() : "";
  if (!governorate) {
  ...
  const parsedQty =
    typeof rawQty === "number" ? rawQty : parseInt(String(rawQty || "1"), 10);
  if (isNaN(parsedQty) || !Number.isInteger(parsedQty) || parsedQty < 1) {
    return { valid: false, error: "يرجى تحديد كمية صحيحة (1 أو أكثر)." };
  }

  const color =
    typeof payload.selectedColor === "string"
      ? payload.selectedColor.trim()
      : "";
  ...
  const orderId = `MAX-${Date.now().toString().slice(-6)}-${Math.floor(
    100 + Math.random() * 900
  )}`;
...
/**
 * Sends Telegram notification using official Telegram Bot API sendMessage.
 * Tries formatted HTML first; if parsing fails, automatically retries as plain text.
 */
export async function sendTelegramOrderNotification(
  ...
  if (!botToken || !chatId) {
    console.error(
      "[Telegram Error] TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID is missing from environment variables."
    );
    return {
      ok: false,
      error:
        "إعدادات إشعار تيليجرام غير مكتملة على الخادم (TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID).",
    };
  }

  const notesHtml = order.notes
    ? escapeTelegramHtml(order.notes)
    : "لا توجد ملاحظات";
  ...
  try {
    ...
    console.warn(
      "[Telegram HTML Attempt Failed] Retrying as plain text...",
      data?.description || response.status
    );

    // 2. Retry with plain text without parse_mode
    ...
    if (fallbackResponse.ok && fallbackData?.ok) {
      console.log(
        `[Telegram Success] Plain text order notification sent for ${order.id}`
      );
      return { ok: true };
    }

    const errorDetail =
      fallbackData?.description || data?.description || `HTTP ${fallbackResponse.status}`;
    console.error(`[Telegram Error] API call failed: ${errorDetail}`);
    ...
    console.error("[Telegram Network Error]", err.message || err);
    return {
      ok: false,
      error: "تعذر الاتصال بخوادم تيليجرام. يرجى التحقق من اتصال الإنترنت.",
    };
  }
}

/**
 * Universal handler used by both Express (server.ts) and Vercel Serverless Function (api/order.ts)
 */
export async function handleOrderSubmission(
  req: any,
  res: any,
  onSuccess?: (order: ValidatedOrder) => void
): Promise<void> {
  ...
  // Parse body safely (supports parsed object, JSON string, or stream)
  let body = req.body;
  if (!body && typeof req.on === "function") {
    try {
      ...
      console.warn("Could not read request stream:", streamErr);
    }
  ...
  if (!telegramResult.ok) {
    // If Telegram notification fails, do NOT return fake success
    sendJson(502, {
      success: false,
      error:
  ...
  // 3. Optional local persistence callback
  if (onSuccess) {
    try {
      onSuccess(order);
    } catch (cbErr) {
      console.warn("Error running order success callback:", cbErr);
    }
  }
